...
public:
    ...
    const std::map<std::string, double> GetStats() const {
        ...
        // calculate avg of runtime
        double avg_time =
            remain_time
            / static_cast<double>(time_of_runtime_.size() - warmup_iter);

        // calculate std of runtime
        double std_time = 0.0;
        for(std::size_t i = warmup_iter; i < time_of_runtime_.size(); ++i) {
            std_time += (time_of_runtime_[i] - avg_time)
                        * (time_of_runtime_[i] - avg_time);
        }
        std_time = std::sqrt(
            std_time
            / static_cast<double>(time_of_runtime_.size() - warmup_iter));

        // calculate min and max of runtime
        double min_time = time_of_runtime_[warmup_iter];
        double max_time = time_of_runtime_[warmup_iter];
        for(std::size_t i = warmup_iter; i < time_of_runtime_.size(); ++i) {
            if(time_of_runtime_[i] < min_time) {
                min_time = time_of_runtime_[i];
            }
            if(time_of_runtime_[i] > max_time) {
                max_time = time_of_runtime_[i];
            }
        }

        // construct stats info dict
        ...
        stats_info_of_runtime_dict["std_time"] = std_time;
        stats_info_of_runtime_dict["min_time"] = min_time;
        stats_info_of_runtime_dict["max_time"] = max_time;
    ...
    void PrintInfo(const std::map<std::string, double> &stats,
        ...
        AINFO_F("Average FPS (w/o warmup step): {}.",
                1.0 / stats.at("avg_time"));
        AINFO_F("Standard deviation of runtime (w/o warmup step): {}ms.",
                stats.at("std_time") * 1000);
        AINFO_F("Min time of runtime (w/o warmup step): {}ms.",
                stats.at("min_time") * 1000);
        AINFO_F("Max time of runtime (w/o warmup step): {}ms.",
                stats.at("max_time") * 1000);
        AINFO_F("============= Runtime Stats Info ({}) =============", prefix);
    }
